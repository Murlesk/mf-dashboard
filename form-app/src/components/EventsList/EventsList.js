...
function EventsList() {
  ...
  const filteredEvents = {
    past: events.past.filter(event => {
      ...
      const matchesDebt = !showOnlyDebt || (event.debt && parseFloat(event.debt) > 0);
  ...
  if (loading) return <div className={styles.loading}>Загрузка мероприятий...</div>;
    ...
    <div className={styles.eventsContainer}>
      ...
      <div className={styles.searchPanel}>
        ...
        <button
          ...
          className={`${styles.debtFilterButton} ${showOnlyDebt ? styles.active : ''}`}
        >
      ...
      {(searchTerm || showOnlyDebt) && (
        <div className={styles.searchStats}>
          ...
          <span className={styles.statsNumbers}>
            <span className={styles.pastStat}>{filteredEvents.past.length} прошлых</span>
            <span className={styles.currentStat}>{filteredEvents.current.length} текущих</span>
            <span className={styles.futureStat}>{filteredEvents.future.length} будущих</span>
          </span>
        </div>
      )}

       {/* ✅ Текущие и будущие мероприятия - постоянные колонки */}
      <div className={styles.currentFutureContainer}>
        {/* Текущие мероприятия */}
  ...
  {event.debt !== undefined && event.debt !== null && event.debt > 0 && (
  <div className={styles.debtBadge}>
    Задолженность: <strong>{parseFloat(event.debt).toLocaleString('ru-RU', {
      ...
      maximumFractionDigits: 2
                  ...
                  <p><strong>Дата начала:</strong> {new Date(event.dateBegin).toLocaleDateString('ru-RU')}</p>
                  <p><strong>Дата окончания:</strong> {new Date(event.dateEnd).toLocaleDateString('ru-RU')}</p>
                  <p><strong>Место:</strong> {event.location}</p>
                </div>
              ))
            ) : (
              <p className={styles.noEvents}>
                {showOnlyDebt ? 'Нет текущих мероприятий с задолженностью' : 'Нет текущих мероприятий'}
              </p>
            )}
          </div>
        </div>
  ...
  <div className={styles.debtBadge}>
    Задолженность: <strong>{parseFloat(event.debt).toLocaleString('ru-RU', {
      ...
      maximumFractionDigits: 2
              ...
              <p className={styles.noEvents}>
                {showOnlyDebt ? 'Нет будущих мероприятий с задолженностью' : 'Нет будущих мероприятий'}
              </p>
            )}
          </div>
        </div>
      ...
      <div className={styles.section}>
        <div 
          className={styles.sectionHeaderPast} 
  ...
  <p><strong>Место:</strong> {event.location}</p>
              ...
              <p className={styles.noEvents}>
                {showOnlyDebt ? 'Нет прошлых мероприятий с задолженностью' : 'Нет прошлых мероприятий'}
              </p>
            )}
          </div>
        )}
